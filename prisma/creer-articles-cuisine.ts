/**
 * Crée les articles de la feuille Cuisine absents du catalogue.
 *
 * Les unités suivent celles de leur section : les fromages au kilo, les épices
 * au kilo.
 */
...
const A_CREER = [
  { name: 'ROQUEFORT', categorie: 'FROMAGE ET LAITIERS', unite: 'kg' },
  { name: 'SAFRAN',    categorie: 'EPICES',              unite: 'kg' },
]
...
async function main() {
  ...
  for (const a of A_CREER) {
    ...
    if (exists) {
      console.log(`${a.name.padEnd(12)} existe déjà`)
      continue
    }
    ...
    console.log(`${p.name.padEnd(12)} créé — réf ${p.reference}, ${a.unite}, ${a.categorie}`)
  }
}
