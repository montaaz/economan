...
/**
 * Rend en PDF la feuille de tournée d'un passage.
 *
 * Elle liste ce qui reste dû au département, toutes commandes confondues, avec
 * une colonne vide : on y note au stylo ce qui sort du magasin, et la saisie à
 * l'écran vient après, d'après ces notes.
 */
export async function GET(request: Request) {
  ...
  try {
    ...
    await chargerFeuille(page, `${base}/economat/services/feuille?${p}`)

    const pdf = await rendrePdf(page, `Feuille de servi — ${departement.code} — ${jour ?? ''}`.trim())

    return new NextResponse(new Uint8Array(pdf), {
      headers: {
        'content-type': 'application/pdf',
        'content-disposition':
          `inline; filename="${departement.code}-feuille-${rang}.pdf"`,
